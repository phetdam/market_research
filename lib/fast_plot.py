"""
module for fast graphing of multiple series generically. makes matplotlib a
little more tractable and removes the need for multiple lines or a deep
understanding its object hierarchy.
...
>>> fmt = ["m.", "g-", "c^"]
>>> fp.xy_plot((a, b, c), ll__ = ["cheese", "bacon", "germs"], fmt_ = fmt,
... fout = "xy_plot.png", title = "silly graph", xlab = "axis of folly",
... ylab = ":)", tilt_y = 0)
"""
# Changelog:
#
# 01-03-2019
#
# separated module description from change log and made module description and
# the sample usage a docstring instead of a commented block. changed all
# function names and the library name to all be private variables. changed
# xy_plot comment block to docstring as well.
#
# 12-22-2018
#
# as i have done with some of the other files, changed line lengths to 80 chars
# or less. emacs' whitespace mode is great.
#
# 12-20-2018
#
# added error checking to make sure that appropriate exceptions are raised when
# there the given formats are zero characters in length
#
# 12-19-2018
#
# changed XY_PLOT_N to "xy_plot" to reflect function name change. added ability
# to specify either a broad type of plot format with a single string, or a list
# to specify individual formats for each single xy-series. changed __IMG__EXTS
# to _IMG_EXTS; did not know about the name mangling before. updated silly
# sample usage.
#
# 12-09-2018
#
# initial creation. method allows for plotting multiple pairs of series, setting
# the x, y, title, and legend labels, tilting the y axis an arbitrary number of
# degrees, changing the font size of x, y, and title, saving to file, and
# changing the width and height of the plot. not comprehensive, but has enough
# features for basic needs.
...
def xy_plot(gs__, w = 8, h = 4.5, fout = None, ll__ = None, fmt_ = AUTO_FORMAT,
    ...
    """
    graphing function; creates a plot and saves it to specified .png file.
    dimensions, x and y labels, and title may be specified. formatting cannot be
    controlled except for whether or not the y axis label is vertical/horizontal,
    which can be made vertical with the option tilt_y (by default 90, so the y
    axis will be vertical). legend is automatically determined, but it is optional
    to pass manual specifications for each xy-series or a single specification
    for all the series as a whole.
    ...
    gs__        list of series pairs to graph; each pair must be in the form of
                (x_series, y_series) or [x_series, y_series]; both x_series and
                y_series in one tuple/list. full list would be:

                [[x_1, y_1], [x_2, y_2] ...]

                each tuple's x and y series must have matching lengths.
    w, h        width and height of image (default w = 8, h = 4.5)
    fout        output file, default None; if None, the figure will not be written
                to an image file.
    ll__        list of legend labels; default ll__ = None. if None, will label
                each line in the legend as "plot_k" where k is the position of
                the tuple in qs__
    fmt_        format for either each individual xy-series or for all the series.
                the default value for fmt_ is specified by AUTO_FORMAT
                starting from 0. len(ll__) must equal len(gs__).
    xlab        x label, default xlab = "default_xlab"; make empty by passing None
    ylab        y label, default ylab = "default_ylab"; make empty by passing None
    title       title, default title = "default_title"; make empty by passing None
    ...
    if (fout != None):
        ...
        if (len(fout_s) != 2 or
            ...
            raise TypeError("{0}.{1}: error: file type restricted to {2}".format(
                _LIBNAME, _XY_PLOT_N, _IMG_EXTS))
    # first check if gs__ is iterable, but not a string
    try:
        iter(gs__)
        if (isinstance(gs__, str)):
            raise TypeError("{0}.{1}: error: required argument must be "
                            "non-string iterable".format(_LIBNAME, _XY_PLOT_N))
    except TypeError:
        raise TypeError("{0}.{1}: error: required argument must be "
                        "iterable".format(_LIBNAME, _XY_PLOT_N))
    ...
    for e in gs__:
        ...
        for ee in e:
            try:
                ...
                if (isinstance(e, str)):
                    raise TypeError("{0}.{1}: error: non-string iterable "
                                    "expected in element".format(_LIBNAME,
                                                                 _XY_PLOT_N))
            except TypeError:
                raise TypeError("{0}.{1}: error: iterable expected in "
                                "element".format(_LIBNAME, _XY_PLOT_N))
        # if e is not length 2, we got a problem
        if (len(e) != 2):
            raise IndexError("{0}.{1}: error: iterable element must have "
                             "length 2".format(_LIBNAME, _XY_PLOT_N))
        # if it is length 2 but the series not of the same length, raise error
        if (len(e[0]) != len(e[1])):
            raise IndexError("{0}.{1}: error: x and y series of element must "
                             "have same length".format(_LIBNAME, _XY_PLOT_N))
    # if ll__ is not None and gs__ and ll__ are not the same length, raise error
    if ((ll__ != None) and (len(gs__) != len(ll__))):
        raise IndexError("{0}.{1}: error: number of labels must equal number of "
                         "plotted series".format(_LIBNAME, _XY_PLOT_N))
    ...
    if (fmt_ != AUTO_FORMAT):
        ...
        try:
            ...
            if (isinstance(fmt_, str)):
                ...
                if (len(fmt_) < 1):
                    raise IndexError("{0}.{1}: error: length of non-iterable "
                                     "fmt_ must be positive".format(_LIBNAME,
                                                                    _XY_PLOT_N))
                # if fmt_ (str) is more than 2 characters, raise error
                if (len(fmt_) > 2):
                    raise IndexError("{0}.{1}: error: length of non-iterable "
                                     "fmt_ must be two characters or "
                                     "less".format(_LIBNAME, _XY_PLOT_N))
                # if all tests pass, set is_fmt_iterable to False
                is_fmt_iterable = False
            # else fmt_ is a non-string iterable
            else:
                # for each element, make sure it is a string of length 2 max
                # (defer checking if the formats are correct to the matplotlib)
                for e in fmt_:
                    # if e is not a string
                    if (not isinstance(e, str)):
                        raise TypeError("{0}.{1}: error: elements of iterable "
                                        "fmt_ must be of type str".format(
                                            _LIBNAME, _XY_PLOT_N))
                    # if length of e < 1
                    if (len(e) < 1):
                        raise IndexError("{0}.{1}: error: elements of iterable "
                                         "fmt_ must be at least one character"
                                         "".format(_LIBNAME, _XY_PLOT_N))
                    # if length of e > 2
                    if (len(e) > 2):
                        raise IndexError("{0}.{1}: error: elements of iterable "
                    ...
                    if (len(fmt_) != len(gs__)):
                        raise IndexError("{0}.{1}: error: number of format "
                                         "specifiers must equal number of "
                                         "plotted series".format(_LIBNAME,
                                                                 _XY_PLOT_N))
        ...
        # else fmt_ is not iterable; raise type error
            ...
            raise TypeError("{0}.{1}: error: non-iterable fmt_ must be type of "
                            "str".format(_LIBNAME, _XY_PLOT_N))
    ...
    if (is_fmt_iterable == True):
        # plot all xy series, using fmt_ (let library catch invalid formats)
    ...
    # set y label (rotate by 90 or 0); use default font size if None fontsize_y
    if (fontsize_y == None):
...
if (__name__ == "__main__"):
    print("{0}: do not run in standalone mode.".format(_LIBNAME))
